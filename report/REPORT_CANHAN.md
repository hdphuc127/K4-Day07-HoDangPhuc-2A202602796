...
> Sau khi cả nhóm cùng chuyển sang `OpenAIEmbedder` thật, `benchmark/bench.py` (dùng `RecursiveChunker`) đạt 4/5 câu có gold marker trong top-3 — cao hơn 3/5 của `HeadingChunker` phẳng cá nhân, nhưng bằng đúng 4/5 khi mình thêm tầng roll-up hierarchical lên trên `HeadingChunker`. Điều này cho thấy với các tài liệu ngắn/vừa như corpus học bổng (500-6000 ký tự), việc chunk theo heading không nhất thiết tối ưu hơn recursive ở dạng phẳng; nhưng khi kết hợp thêm một tầng tóm tắt LLM (roll-up), chính "ranh giới sạch theo heading" ở tầng gốc lại giúp bản tóm tắt gom đúng chủ đề, kéo kết quả retrieval trở lại ngang bằng recursive.
